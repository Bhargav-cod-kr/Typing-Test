...
function removeClass(el, name) { el.className = el.className.replace(new RegExp('\\b' + name + '\\b', 'g'), ''); }
...
function getWpm() {
  ...
  const lastIndex = wordsEl.indexOf(lastTypedWord) + 1;
  const typedWords = wordsEl.slice(0, lastIndex);
  const correctWords = typedWords.filter(word => [...word.children].every(letter => letter.className.includes('correct')));
...
function startTimer() {
  if (!window.timer) {
    window.timer = setInterval(() => {
      ...
      const elapsed = Date.now() - window.gameStart;
...
document.getElementById('game').addEventListener('keyup', ev => {
  ...
  if (isLetter || isSpace) startTimer();
  ...
  // Space handling
  if (isSpace) {
    if (expected !== ' ') [...currentWord.querySelectorAll('.letter:not(.correct)')].forEach(l => addClass(l, 'incorrect'));
